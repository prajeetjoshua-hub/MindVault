...
128 regression tests passed; typecheck and both web exports passed. Browser verified the visible Qwen and dashboard connection states, deterministic reply labels, Enter-to-send, Enter-to-create-PIN, tab-session restoration, live trace delivery, the compact landscape dashboard, food and relationship context handling, breakup and self-harm context handling, privacy wording for “seeing/viewing” questions, context-first clarification, explicit reassurance ending the pending loop, return to listening, help cards and SOS confirmation without placing a call. Model gating preserves specific LOW authored replies while allowing Qwen3 for eligible MEDIUM or generic LOW turns; safety and uncertainty remain blocked. Qwen3 4B Instruct 2507 passed the focused desktop transcript review; Gemma 1B and Qwen2.5 3B did not. Native compilation, encrypted-vault inspection and Samsung F15 testing remain pending. See PROGRESS-2026-09-13.md for scope and remaining device checks.

## 14 September 2026 recovery check

Windows reported paging-file exhaustion while the model, tests and web tooling were running concurrently; desktop composition and local preview processes were interrupted. The Qwen artifact itself was not damaged. The launcher now uses quantised key/value caches and skips warm-up to reduce its memory peak, while keeping the 4,096-token model context used by this prototype. A stopped or restarted runtime invalidates the browser's stale connection, updates the settings status and falls back to authored wording that does not expose model plumbing.

129 regression tests, TypeScript checking and the functional browser export passed after the change while Qwen remained loaded. A live context-specific prompt produced a relevant response visibly labelled **LOCAL QWEN REPLY**. This verifies the repaired desktop connection path; it is not clinical validation or Samsung F15 acceptance testing.
